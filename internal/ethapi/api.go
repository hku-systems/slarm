...
//bccdn tx transfer
func (args *SendTxArgs) BtoTransaction(num uint32, nonce uint64) *types.Transaction {
	var input []byte
	if args.Input != nil {
		input = *args.Input
	} else if args.Data != nil {
		...
		return types.NewTransactionBCCDN(nonce, *args.To, (*big.Int)(args.Value), uint64(*args.Gas), (*big.Int)(args.GasPrice), bs)
	}
	//actually not used
	fmt.Println(input)
	return  types.NewTransactionBCCDN(nonce, *args.To, (*big.Int)(args.Value), uint64(*args.Gas), (*big.Int)(args.GasPrice), input)
}

// SubmitTransaction is a helper function that submits tx to txPool and logs a message.
func SubmitTransaction(ctx context.Context, b Backend, tx *types.Transaction) (common.Hash, error) {
	tx.SetUrgentNonce(666)
	if tx.Nonce() % 2 == 0 {
		tx.SetUrgent(true)
	// } else {
	// 	tx.SetUrgent(false)
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	//for i := 1; i <= 2; i++ {
		fmt.Println("called in loop")
		//tx := args.BtoTransaction(uint32(i), uint64(i))
		tx := args.toTransaction()
		...
		h, _ := SubmitTransaction(ctx, s.b, signed)
		fmt.Println("tx hash: ", h.Hex())
	//}
