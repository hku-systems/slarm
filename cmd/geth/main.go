...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	fmt.Print("tianxiang test log, before start node \n")
	// Start up the node itself
	utils.StartNode(stack)
	fmt.Print("tianxiang test log, before unlock accounts \n")
	...
	go func() {
		...
		fmt.Println(accounts.CallAccount())
		
		// Open any wallets already attached
		for _, wallet := range stack.AccountManager().Wallets() {

			fmt.Print("tianxiang test log, into wallet open \n")
		...
		for event := range events {
			...
			fmt.Print("tianxiang test log, into wallet switch \n")

			switch event.Kind {
			case accounts.WalletArrived:

				fmt.Print("tianxiang test log, into wallet arrived \n")
			...
			case accounts.WalletOpened:
				...
				fmt.Print("tianxiang test log, into wallet opened \n")
			...
			case accounts.WalletDropped:
				...
				fmt.Print("tianxiang test log, into wallet dropped \n")
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		fmt.Println("out of if: gasprice: ", gasprice)
		if ctx.IsSet(utils.MinerGasPriceFlag.Name) {

			gasprice = utils.GlobalBig(ctx, utils.MinerGasPriceFlag.Name)

			fmt.Println("in if: gasprice: ", gasprice)
		}
...
func unlockAccounts(ctx *cli.Context, stack *node.Node) {
	...
	for _, input := range inputs {
		if trimmed := strings.TrimSpace(input); trimmed != "" {
			fmt.Println("unlock account: ", input)
