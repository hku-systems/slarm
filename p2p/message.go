...
func Send(w MsgWriter, msgcode uint64, data interface{}) error {
	...
	fmt.Println("Qi Ji in message.go/Send()", "size", size, "msgCode", msgcode)
	if err != nil {
		return err
	}
...
func (p *MsgPipeRW) WriteMsg(msg Msg) error {
	fmt.Println("Qi Ji in WriteMsg start!!", "Code", msg.Code, "Size", msg.Size)
	if atomic.LoadInt32(p.closed) == 0 {
		consumed := make(chan struct{}, 1)
		msg.Payload = &eofSignal{msg.Payload, msg.Size, consumed}
		fmt.Println("Qi Ji in WriteMsg", "Code", msg.Code, "Size", msg.Size)
		select {
		case p.w <- msg:
			if msg.Size > 0 {
				...
				select {
				case <-consumed:
					fmt.Println("Qi Ji in WriteMsg message is consumed", "Code", msg.Code, "Size", msg.Size)
				case <-p.closing:
				}
			}
			return nil
...
func (p *MsgPipeRW) ReadMsg() (Msg, error) {
	if atomic.LoadInt32(p.closed) == 0 {
		select {
		case msg := <-p.r:
			fmt.Println("Qi Ji in ReadMsg", "code", msg.Code, "Size", msg.Size)
			return msg, nil
		case <-p.closing:
		}
	}
...
func (ev *msgEventer) WriteMsg(msg Msg) error {
	fmt.Println("Qi Ji in msgEventerWriteMsg start!!", "Code", msg.Code, "Size", msg.Size)
	err := ev.MsgReadWriter.WriteMsg(msg)
	fmt.Println("Qi Ji in msgEventerWriteMsg  1", "error", err.Error())
	if err != nil {
		return err
	}
	ev.feed.Send(&PeerEvent{
