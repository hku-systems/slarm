...
func (sm *SafeMap) Store(id enode.ID, cap string) {
	sm.Lock()
	defer sm.Unlock()
	sm.m[fmt.Sprintf("%x", id.Bytes()[:8])] = cap
}

func (sm *SafeMap) Load() map[string] string{
	sm.RLock()
	defer sm.RUnlock()
	m_new:= make(map[string]string)
...
func (sm *SafeMap) Delete(id enode.ID) {
	sm.Lock()
	defer sm.Unlock()
	delete(sm.m, fmt.Sprintf("%x", id.Bytes()[:8]))
}


var (
	PeerCapsMap = SafeMap{m: make(map[string]string)}
)
	...
	for {
		...
		case c := <-srv.checkpointAddPeer:
			...
			if err == nil {
				...
				p := newPeer(srv.log, c, srv.Protocols, srv.LocalNode())
				fmt.Println("Qi Ji generated a new peer, set PeerCapsMap as 000")
				PeerCapsMap.Store(p.ID(), "000")
				...
				fmt.Println("Qi Ji in adding p2p peer", "addr", p.RemoteAddr(), "peers", len(peers)+1, "name", name)
				go srv.runPeer(p)
				peers[c.node.ID()] = p
				if p.Inbound() {
					inboundCount++
...
func (srv *Server) maxDialedConns() int {
	...
	fmt.Println("Qi Ji in server.go/maxDailedConns()/MaxPeers", srv.MaxPeers, "r", r)
...
func (srv *Server) listenLoop() {
	...
	for {
		...
		for {
			fmt.Println("Qi Ji in server.go/listenLoop, new connection")
...
func nodeFromConn(pubkey *ecdsa.PublicKey, conn net.Conn) *enode.Node {
	...
	if tcp, ok := conn.RemoteAddr().(*net.TCPAddr); ok {
		ip = tcp.IP
		port = tcp.Port
		fmt.Println("Qi Ji in nodeFromConn")
	}
...
func (srv *Server) runPeer(p *Peer) {
	fmt.Println("Qi Ji in server.go/runPeer()")
