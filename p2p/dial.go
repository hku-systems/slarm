...
func (s *dialstate) newTasks(nRunning int, peers map[enode.ID]*Peer, now time.Time) []task {
	...
	fmt.Println("Qi Ji in dail.go", "maxDynDials", s.maxDynDials, "Peers", len(peers), "needBynDails", needDynDials)
	...
	if len(s.lookupBuf) < needDynDials && !s.lookupRunning {
		fmt.Println("Qi Ji in dail.go: Launch a discovery lookup if more candidates are needed")
